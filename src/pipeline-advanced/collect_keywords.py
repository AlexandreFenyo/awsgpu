...
------------------------------------------------------------
 ...
 - --order freq: Sorts keywords by how often they appear across chunks (global frequency). Each chunk contributes at most +1 per keyword, so this is closer to “in how many chunks does this phrase occur?” than raw term frequency. Tie-breaker:   
   longer phrase first.
 - --order specificity: Sorts by an internal per-phrase score that tries to reflect how “specific/important” a phrase is within a chunk. The script keeps the best (highest) score seen for each phrase across all chunks and sorts by that.

What goes into the “specificity” score
 - Named entities: +2.0, or +3.0 if label is ORG/PRODUCT/WORK_OF_ART/MISC.
 - Quoted or Markdown-emphasized names: +2.5.
 - Noun phrases (NPs): +1.5.
 - Frequent content 1–3-grams (NOUN/PROPN/ADJ): +1.0 per occurrence window.
 - Length boost: +0.25 for each extra token in a phrase (slight preference for bi/tri-grams).

Implications

 - freq favors globally common terms across your dataset (good for “what’s talked about most?”).
 - specificity favors phrases that look like concrete entities or meaningful NP phrases inside a chunk (good for “most informative/specific” items). It’s heuristic—not a strict linguistic “specificity” or TF-IDF.
"""
